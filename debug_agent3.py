"""
Debug version of the agent with proper Unicode handling
"""
...
def run_agent_loop_debug(user_query: str, max_iterations: int = 10) -> str:
    """
    Run the agent loop where the LLM decides which tools to call - with debug output.
    ...
    safe_print("DEBUG: Sending system prompt")
    chat.send_message(system_prompt)

    # Add user query
    safe_print(f"DEBUG: Sending user query: {user_query}")
    ...
    for iteration in range(max_iterations):
        ...
        try:
            ...
            safe_print(f"DEBUG: function_calls = {function_calls}")

            if function_calls:
                safe_print(f"DEBUG: Processing {len(function_calls)} function calls...")
                # Process tool calls
                function_responses = []
                ...
                for fc in function_calls:
                    ...
                    safe_print(f"DEBUG: Tool call - {tool_name} with input: {tool_input}")
                    ...
                    # Safe print of tool result
                    result_str = json.dumps(tool_result, indent=2)
                    safe_print(f"DEBUG: Tool result: {result_str}")
                ...
                safe_print(f"DEBUG: Sending {len(function_responses)} function responses back to model")
                response = chat.send_message(function_responses)
                response_text = getattr(response, 'text', '') or ''
                safe_print(f"DEBUG: Got response. Text: '{response_text}'")
                safe_print(f"DEBUG: Response has function_calls: {bool(getattr(response, 'function_calls', None))}")
                continue

            # If we get here, there are no function calls, so we have a final answer
            safe_print("DEBUG: No function calls found, returning final answer")
            final_answer = getattr(response, 'text', '') or ''
            safe_print(f"DEBUG: Final answer: '{final_answer}'")
            return final_answer

        except Exception as e:
            safe_print(f"DEBUG: Error in iteration {iteration}: {str(e)}")
            import traceback
            traceback.print_exc()
...
if __name__ == "__main__":
    ...
    safe_print("Testing agent loop with query:", test_query)
    safe_print("=" * 50)
