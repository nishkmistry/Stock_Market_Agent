"""
Debug version of the agent to see what's happening in each iteration
"""
...
def run_agent_loop_debug(user_query: str, max_iterations: int = 10) -> str:
    ...
    print("DEBUG: Sending system prompt")
    chat.send_message(system_prompt)

    # Add user query
    print(f"DEBUG: Sending user query: {user_query}")
        ...
        try:
            ...
            if function_calls:
                ...
                for fc in function_calls:
                    ...
                    print(f"DEBUG: Tool call - {tool_name} with input: {tool_input}")
                ...
                print(f"DEBUG: Got response. Text: '{response.text}'")
                print(f"DEBUG: Response has function_calls: {bool(getattr(response, 'function_calls', None))}")
                continue

            # If we get here, there are no function calls, so we have a final answer
            print("DEBUG: No function calls found, returning final answer")
            final_answer = response.text
            print(f"DEBUG: Final answer: '{final_answer}'")
            return final_answer

        except Exception as e:
            print(f"DEBUG: Error in iteration {iteration}: {str(e)}")
            import traceback
            traceback.print_exc()
